...
static bool json_is_array_with_tokens(const json & data) {
    if (data.is_array()) {
        for (const auto & e : data) {
            if (e.is_number_integer()) {
                return true;
 ...
 * - "prompt": { "prompt": "string", "multimodal_data": [ "base64" ] }
 */
static server_tokens tokenize_input_subprompt(const llama_vocab * vocab, mtmd_context * mctx, const json & json_prompt, bool add_special, bool parse_special) {
    ...
    if (json_prompt.is_string() || json_is_array_of_mixed_numbers_strings(json_prompt)) {
        ...
        if (has_mtmd && json_prompt.find("multimodal_data") != json_prompt.end()) {
            ...
            return process_mtmd_prompt(mctx, json_prompt.at("prompt"), files);
        } else {
            // Not multimodal, but contains a subobject.
            llama_tokens tmp = tokenize_mixed(vocab, json_prompt.at("prompt"), add_special, parse_special);
            return server_tokens(tmp, false);
        }
   } else {
       throw std::runtime_error("\"prompt\" must be a string, an list of tokens, a JSON object containing a prompt, list of mixed strings & tokens, or a mixed format list");
   }
}
 ...
 * - "prompt": [[12, 34, "string", 56, 78], [12, 34, 56], { "prompt": "string", "multimodal_data": [ "base64" ]}]
 */
